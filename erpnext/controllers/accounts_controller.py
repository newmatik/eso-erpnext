...
def update_child_qty_rate(parent_doctype, trans_items, parent_doctype_name):
    ...
    for d in data:
        ...
        if parent_doctype == 'Purchase Order':
            if child_item.received_qty > d.get("qty"):
                frappe.throw(_("Row #{0}: Cannot set Qty if greater than Received Qty for Item {1}.")
                    .format(child_item.idx, child_item.item_code))
            else:
                child_item.qty = float(d.get("qty"))
        else:
            if child_item.delivered_qty > d.get("qty"):
                frappe.throw(_("Row #{0}: Cannot set Qty if greater than Delivered Qty for Item {1}.")
                    .format(child_item.idx, child_item.item_code))
            else:
                child_item.qty = float(d.get("qty"))
